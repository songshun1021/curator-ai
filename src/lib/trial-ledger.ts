...
export function getPlatformTrialConfig(): PlatformTrialConfig {
  return {
    ...
    model: process.env.PLATFORM_MODEL?.trim() || "qwen-plus",
    ...
    inputCostPerMillionCny: getConfigNumber("TRIAL_INPUT_COST_PER_MILLION_CNY", 0.8),
    outputCostPerMillionCny: getConfigNumber("TRIAL_OUTPUT_COST_PER_MILLION_CNY", 2),
  };
}
...
export async function reserveTrialRequest(trialId: string, ip: string, context: string): Promise<TrialAllowanceResult> {
  ...
  const blockedReason =
    ...
    (ipCount >= config.hourlyIpLimit ? "当前访问过于频繁，请稍后再试。" : null) ||
  ...
  if (blockedReason) {
    return { ok: false, reason: blockedReason, statusCode: ipCount >= config.hourlyIpLimit ? 429 : 402 };
  }
