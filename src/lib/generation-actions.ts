  ...
  try {
    ...
    if (docType === "custom-resume") {
      ...
      if (customWarnings.length > 0) {
        ...
        await addSystemNotice(`定制简历已保存，但内容仍偏空：${customWarnings.map((item) => `「${item}」`).join("、")}。${resumeHint}`);
      }
      if (fitAnalysis.fitWarnings.length > 0) {
        await addSystemNotice(`定制简历已保存，但贴岗改写仍偏弱：${fitAnalysis.fitWarnings.map((item) => `「${item}」`).join("、")}。建议补充更相关事实后重新生成。`);
      }
      await finishGeneration("done");
